...
def create_meal(meal: str, cuisine: str, price: float, difficulty: str) -> None:
    ...
    Raises:
        ValueError: If price is not int or float, or less than 0
        ValueError: If difficulty is not 'LOW', 'MED' or 'HIGH'
        ValueError (From sqlite3.IntegrityError): If the same meal value already exists
...
def delete_meal(meal_id: int) -> None:
    ...
    Raises:
        ValueError: If the meal with the given ID is already marked as deleted
        ValueError (From TypeError): If the meal with the given ID is not found
...
def get_leaderboard(sort_by: str="wins") -> dict[str, Any]:
    """
    Gets a list of the elements in the database, ordered by their win percentage or win count

    Args:
        sort_by (str): What to sort the output by, "wins" by default

    Returns:
        leaderboard (dict[str, Any]): The sorted list
...
def update_meal_stats(meal_id: int, result: str) -> None:
    """
    Increments the battle count and win count (if it won) of a meal by meal_id.

    Args:
        meal_id (int): The ID of the meal whose battle count and win count are to be incremented
        result (str): Whether or not the meal won
